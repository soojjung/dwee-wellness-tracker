...
## `.claude/rules/screens.md` §1 fullscreen route list — resynced 2026-09-09

Fixed the gap flagged 2026-09-08: §1 now lists all 12 routes found via `find src/app/\(fullscreen\) -name page.tsx` (includes `/home/customize/edit-photos/[slot]`, which even the earlier flagged-gap note missed). Re-run that find command rather than trusting any doc's route list, including this memory note, next time it needs checking — it drifts fast.

## AuthGuard PUBLIC_PREFIXES exception (2026-09-09) — first hole in the MVP2 login gate

`src/components/auth/AuthGuard.tsx` gained a `PUBLIC_PREFIXES` whitelist (currently just `/magazine/personal-body-type/share`) that bypasses the session check entirely — pages reachable from a link the app itself handed out. Documented in three places now: `docs/flows/diagnose.md` §"공유 (Share)" (full mechanics + updated Mermaid, since this route is the only consumer), `.claude/rules/screens.md` §1 (one-line policy note + pointer to diagnose.md, added right after the route-group list since the exception cuts across route groups rather than belonging to one), and `README.md` (both the sync-strategy paragraph and the login-gate decision-table row, plus the diagnose share bullet under "What I Built"). If a second route ever joins this whitelist, update all three, not just the component comment.

Paired change: `ShareLandingRedirect.tsx` previously did an unconditional `router.replace('/magazine/personal-body-type')` (the article) on mount — no auth check at all, and the old Mermaid diagram in `docs/flows/diagnose.md` correctly showed that. As of 2026-09-09 it redirects to `/magazine` (list) instead, and only when `authStore` is hydrated and a user exists — `/magazine` itself sits behind `AuthGuard`, so redirecting a signless visitor there would just bounce them to `/login` a second time. A signed-out visitor now stays on the share-landing screen with a manual link instead. `docs/flows/diagnose.md`'s Mermaid diagram was rewritten with a `세션 있음?` branch to match. README.ko.md's M2.1 roadmap bullet (historical, describes the original article-redirect behavior — was accurate when written) got a forward-pointer parenthetical rather than a rewrite, per the established README.ko.md convention — plus a new dated bullet for the actual fix.

## `.pt-safe`/`.pb-safe` were dead classes until 2026-09-09 — a second precedent for a globals.css rule line

`WithdrawReasonScreen.tsx`, `DiaryCustomizeScreen.tsx`, and `CameraSheet.tsx` had been applying `pt-safe`/`pb-safe` classNames for a while, but grepping `src/app/globals.css` at any point before 2026-09-09 shows those utilities were **never defined anywhere** — Tailwind silently drops unrecognized class names, so safe-area padding had been a no-op the whole time. Fixed by adding them under `@layer utilities` in `globals.css`. Two of the three call sites also needed a non-zero base padding on top of the safe-area inset, so they moved to inline `calc()` instead of keeping the (now-real) bare utility — the `globals.css` comment warns explicitly against stacking a separate padding class next to `pt-safe`, since whichever utility Tailwind emits later in the stylesheet wins and silently cancels the other. Documented as `.claude/rules/screens.md` §11, following the same "globals.css owns app-wide rule → gets a rules.md line" precedent as the `word-break: keep-all` fix (§10, 2026-09-08). Any future latent-no-op-class bug is worth grepping `globals.css`/`tailwind.config.ts` for before assuming a className "already works."

## Diary sticker seeding — versioned + per-backend-scoped as of 2026-09-09

`ensureDefaultStickersSeeded()` in `src/data/index.ts` changed from a single boolean IndexedDB flag to a `DEFAULT_STICKER_SET_VERSION`-keyed, per-backend-scoped flag (`local` vs `remote:{userId}`). Bump the version constant whenever the built-in sticker artwork changes — a device re-seeds only if its library is still empty, so it never resurrects stickers a user deliberately deleted. `rehydrateAll.ts` now also calls `diaryStickerStore.rehydrate()` on every repo-mode switch (6 stores total, not 5), specifically so a newly-signed-in account gets the defaults even if the device's anonymous library was already seeded. Also: the seed loop inserts `DEFAULT_STICKERS` **reversed**, because the sticker adapter's `add()` puts new items first (newest-first) — seeding in array order would render backwards. `defaultStickers.ts` documents this with a code comment; `docs/flows/log.md` and `README.md`/`README.ko.md` all now mention it.

## Diary unified add/edit sheet — AddQuickSheet removed (2026-09-09, feat/diary-unified-entry-sheet)

- `AddQuickSheet.tsx` (the `+` button's [생리 추가]/[일정 추가] 2-item chooser popover) was **deleted**. The `+` button now opens `EventFormSheet` add mode directly — no intermediate chooser. This is a *second* collapse of an intermediate sheet in the diary flow, after the 2026-08-19 `EventDetailSheet` removal noted above; the diary `+`/event-tap paths are now symmetric (both land straight on `EventFormSheet`).
- `EventFormSheet` gained a period toggle in **add** mode too (previously edit-only): local `periodOn` state, disabled when `startDate > today` (future-period guard), applied via `linkPeriodMark` only after a successful `addEvent`. Edit mode's toggle is unchanged (mutates the store immediately).
- New `EventConditionSection.tsx` — optional condition card (mood/energy/pain/bloating/appetite/skin) reusing `ConditionRow` with a **new `outline` variant** (also added to `ChoiceGroup`: hug-content bordered pill vs the original `filled` equal-width pill). Present in both add/edit modes; edit seeds from `conditionByDate[event.startDate]`.
- `conditionStore.upsert` now merges with the existing `byDate[date]` entry before calling the repo, because `conditionRepo.upsert` **replaces the whole record** — any caller that only sets one field (like this sheet) would otherwise wipe `memo`/other fields for that date. Any new caller of `conditionStore.upsert` gets this merge for free; don't re-add a manual merge at the call site.
- Delete button copy/behavior changed: "일정 삭제" → "일정 및 기록 삭제" (new `BinIcon`, new `brand.red` tailwind token `#FF3B30`), and deleting now also calls `unlinkPeriodMark` first if the event has one — the condition log is deliberately left alone (it may hold data logged outside this sheet).
- New shared `src/components/ui/WheelColumn.tsx` extracted from `YearMonthWheelPicker`'s previously-inline wheel implementation — also fixed a scroll-padding centering bug in the extraction (removed `scroll-padding-top`, relies on the top/bottom spacer divs alone). `NotificationsScreen`'s lead-time wheel was migrated onto it too (was tap-only ±1 step before; now proper drag/flick scroll-snap, matching the diary picker).
- `MyPageBackLink` unified to the shared `BackIcon` component (was inline SVG) at 40px (was 36px/`h-9 w-9`). `MyPageToggle` resized to Figma spec (50×24 track / 30×20 thumb, `pink100` on vs `gray400` off — was 48×28/`pink200`/`gray300`) and gained a `disabled` prop, now reused by `EventFormSheet`'s period toggle.
- Docs touched: `docs/flows/log.md` (already updated by the implementing agent — verified accurate against the code above), `docs/flows/settings.md` (wheel-picker description corrected from tap-only to drag/scroll-snap `WheelColumn`), `README.md` (Diary + Event domain bullet + Status paragraph extended), `README.ko.md` (component-tree line fixed — `AddQuickSheet` reference removed — + new dated roadmap bullet appended per the established convention).

## CategorySelector rebuilt as collapsed-row accordion + default category "친구" (2026-09-13)

- `CategorySelector.tsx` changed from an all-chips-visible radio picker to a collapsed row (selected `CategoryChip` + chevron) that joins `EventFormSheet`'s existing date-field accordion group (`ExpandedField` union gained `'category'`) — tapping expands a vertical list with a pink check on the selected row, a per-row "편집" text button, and a trailing "+ 일정 유형 추가" row (Figma 012_2/012_6). `CategoryChip` gained a `size?: 'sm' | 'md'` prop (`sm` = calendar badges, `md` = the new form row/list rows).
- New pure function `defaultCategoryId(categories)` in `src/domain/event/builtins.ts` (+ `DEFAULT_CATEGORY_KEY = 'friend'`, paired `builtins.test.ts`/`builtins.cases.md`) — `EventFormSheet`'s add-mode initial `categoryId` now resolves to the built-in "친구" category (matched by seed `order`, since built-ins carry no key in storage) instead of `categories[0]`.
- This was otherwise a **visual-polish-only diff** alongside the category change: 8px corner radius on photo-source diary stickers (not cutouts) across `PlacedSticker`/`PlacedStickerLayer`/`StickerLibrarySheet`/`DiaryStickerViewLayer`; `StickerScanScreen`'s photo-fit + scan-band redesign (fits both axes, crisp pink line, min display 900ms → 1400ms); `StickerLibrarySheet` header re-centered with equal-width side columns; `YearMonthWheelPicker` header restyled to circular X/✓ + pink underline. None of these needed doc changes on their own — same "polish-only diff, only the behavior change earns a doc touch" call as the 2026-09-04 nav/icon polish PR noted above.
- Docs touched: `docs/flows/log.md` (CategoryChip/CategorySelector bullet rewritten, EventFormSheet bullet gained the default-category sentence, new `domain/event/builtins.ts` bullet added), `README.md` (Diary + Event domain bullet + Diary sticker customization bullet + Status paragraph all extended), `README.ko.md` (new dated roadmap bullet appended after the 2026-09-09 "다이어리 통합 입력 시트" bullet, per the established append-not-rewrite convention).

## DailyConditionLog gained sleep + exercise fields (2026-09-18, migration 0015)

`condition_logs.sleep` (`good|fair|poor`) and `condition_logs.exercise` (`none|light|active`) added via `supabase/migrations/0015_condition_logs_sleep_exercise.sql` — a plain `alter table ... add column` (nullable, no data backfill needed), **not** a bump to `IndexedDBConditionAdapter`'s versioned schema (`migrations.ts` stays at v10 — the local adapter is schemaless/idb-keyval for this key, so new optional `DailyConditionLog` fields don't need a migration entry there, same pattern as the 2026-09-08 body-type-report key). Touches all three condition-input surfaces (`log/LogEntryDialog`, `log/ConditionForm`, `diary/EventConditionSection`+`EventFormSheet`) plus the read-only `diary/eventDetailRows`. Migration commit message cites an "idea contest doc" (감정·증상·식욕·수면·운동·일기) as the source of truth for the field list, which is how sleep/exercise were noticed as gaps — worth remembering if more fields from that same source appear later (e.g. "일기"/diary already exists as memo).

Docs touched: `README.ko.md` (roadmap bullet field list), `docs/flows/log.md` (EventConditionSection "6항목" → "8항목"), `supabase/README.md` (migration tree + both "0001~0014"→"...0015" range mentions — three-spot rule confirmed still accurate), `docs/product/release-plan-v1.md` ("마이그레이션 0001~0014"→"...0015" in the pre-launch DB checklist). **Deliberately left untouched**: `docs/product/mvp1-spec.md`'s condition-enum bullet list and `docs/product/mvp1-remaining-plan.md`'s STEP 9.3 목적 line — both are frozen point-in-time planning docs (their existing mood/energy/etc. value sets already diverge from the current `conditionOptions.ts`, e.g. spec says 기분 has 4 values, code has 5 — proof they were never meant to track implementation 1:1). Treat any future condition-field addition the same way: update README.ko.md/log.md/supabase docs, skip the two historical planning docs, and flag the divergence rather than silently "fixing" old spec numbers. `README.md` (en) has no condition-field enumeration anywhere to update (confirmed by grep) — consistent with the established README.md/README.ko.md "not parallel" precedent.

## PhotoImportModal (album import) vs CameraSheet — ratio-selection timing now diverges (2026-09-09)

The album-import sheet gained a mode-first, two-step flow: pick "누끼" (cutout, now the default) vs "사진 그대로" first via radio cards; only the "그대로" path proceeds to a new full-screen `PhotoRatioScreen` (`src/components/diary-customize/PhotoRatioScreen.tsx`) to choose 1:1/4:3 before cropping. The cutout path skips ratio selection entirely and infers a placement ratio from the source image's own aspect ratio (`ratioForImage`, cutoff 0.875) since a crop before cutout risks slicing the subject. `CameraSheet` (in-app camera capture) was **not** restructured the same way — it still shows ratio + mode toggles together before capture, unchanged. Don't assume the two upload paths are symmetric when documenting or extending either one; `docs/flows/log.md` §"스티커 업로드 분기" explains the asymmetry in prose above the (intentionally still-shared, mode-only) branching diagram rather than drawing both paths in one diagram — doing so would have pushed the diagram past the ~10-node guideline for a detail that didn't earn a dedicated second diagram.

...
/**
 * Email card with copy-to-clipboard — the body of `/settings/qna` and the
 * public `/legal/support` page. Takes the dictionary as a prop because the
 * public page may be pinned to a language with `?lang=`, which `useT()`
 * doesn't know about.
 */
export function SupportContactCard({ t }: { t: Dictionary }) {
    ...
    <>
      <section className="rounded-2xl bg-brand-white px-5 py-6 shadow-[0_1px_2px_0_rgba(0,0,0,0.04)]">
        ...
        <div className="mt-5 flex items-center justify-between gap-3 rounded-xl bg-brand-gray100 px-4 py-3">
          <a href={`mailto:${SUPPORT_EMAIL}`} className="truncate text-sm text-brand-gray900">
            {SUPPORT_EMAIL}
          </a>
          <button
            type="button"

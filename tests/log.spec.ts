...
for (const phase of PHASES) {
  test(`log — ${phase}`, async ({ page }, testInfo) => {
    const locale = testInfo.project.name as Locale;
    const errors = attachErrorGuards(page);

    await seedAndOpenLog(page, phase, locale);

    await expect(page).toHaveScreenshot(`log-${phase}.png`, {
      fullPage: true,
    });

    expect(errors.pageErrors, 'uncaught page errors').toEqual([]);
    expect(errors.consoleErrors, 'console.error during log render').toEqual([]);
  });
}

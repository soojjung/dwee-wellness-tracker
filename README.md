...
- **MyPage (`/settings`)** — auth-card variants (signed-out / signed-in), cycle summary chip using `classifyCycleStatus`, preferences, support link rows, account management. Account edit at `/settings/account` (fullscreen) persists nickname via `supabase.auth.updateUser`; anonymous users bounced. Five sub-page stub routes ready for 015_4–017 designs.

...
- **MyPage (`/settings`)** — auth-card variants (signed-out / signed-in), cycle summary chip using `classifyCycleStatus`, preferences, support link rows, account management. `LogoutConfirmDialog` (pink alert badge) replaces `window.confirm`; post-logout toast uses a module-scope `appToast` queue so the confirmation message survives the route change to `/login`. Language settings screen (`/settings/language`) is live — two radio-style rows, immediate locale switch. Account edit at `/settings/account` (fullscreen) persists nickname via `supabase.auth.updateUser`; anonymous users bounced. Account deletion is a two-screen flow: `WithdrawConfirmDialog` (015_9) → `WithdrawReasonScreen` at `/settings/withdraw` (015_10–14, fullscreen) collects one or more reasons before calling `delete-account`; selections are inserted anonymously into `withdrawal_feedbacks` (no `user_id`) so they survive the deletion cascade. Four remaining sub-page routes (notices/Q&A/terms/privacy) are stubs.
